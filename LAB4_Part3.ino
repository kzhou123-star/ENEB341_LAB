// Assign a name to the digital pin 2; pin 2 connects to the LED.
...
void loop() {
  ...
  while (!done) {
    ...
    if (Serial.available()) {     
      ...
      // TODO 2: inside if statement, check for uppercase letters
      if () {
        Serial.println(ch);
        flash_morse_code(letters[ch - 'A']);        
      }
      // TODO 3: inside else if statement, check for lowercase letters
      else  if () {
        Serial.println(ch);
        flash_morse_code(letters[ch - 'a']);
      }
      // TODO 4: inside else if statement, check for numbers
      else if () {
        Serial.println(ch);
        flash_morse_code(numbers[ch - '0']);
      }
      // TODO 5: inside else if statement, check for space between words
      else if (ch == ' ') {
...
void flash_dot_or_dash(char dot_or_dash) {
  ...
  //TODO 6:
  // Make the LED shine
  // If it is a dot, flash the dot.
  //If it is a dash, flash the dash...equal to three dots
  // Turn the LED off 
  // Give a delay between parts of the same letter...equal to one dot
}
